...
| 模块 | 职责 | 禁止 |
|------|------|------|
| **Collector** | 抓取 raw_evidence；适配器三层降级；验收门 + 自愈补采 | 不做语义分析、不生成结论 |
| **Analyzer** | 填充 feature_tree / pricing_model / user_persona / swot / recommendations；按权重计算 priority_score；缺证据只**声明缺口**（`Gap`）交给 evidence_service.fill | 不抓数据（零采集 import，AST 测试锁边界）；不编 evidence_id；不手写 priority |
| **Writer** | 渲染 Markdown 报告 | 不改 schema |
| **Reviewer** | 跑 R0-R10 规则（R9 chip 可溯源 / R10 禁泄分）；输出 `quality_report`（errors 带 issue 级 `reject_target` 归因，供 checklist/gap_owner 消费） | 只审不修；不写回 state 级打回字段（M4b 已删） |
| **guard** (v3) | 结论强度唯一 owner：`apply()` 幂等终门（幻觉引用清理/G1 强对比对账/G2 basis 对账/泛化收敛）；`guard_revise_node` 消费 Reviewer 发现做一次修订 + 降级分层措辞 | 不新增内容，只降级/删除 |
| **evidence_service** (v3) | 采集执行唯一 owner：`fill(gaps)` 回捞优先→定向外搜；合成访谈 | 不下结论 |
...
- **控制流直线化（v3 M4）**：retry 配额/打回路由/`reject_*` state 字段已删；Reviewer 有 error → `guard_revise` 一次确定性修订定终态（有修订→passed / 零修订→degraded）
...
- **`patch_by_requirements`** 展开所有 `required_claim_types`（不是只取第一个；现为 EvidenceService 侧工具函数，graph 内无调用）
...
- **LLM**：默认豆包 Doubao（`doubao-seed-2-0-lite`）。运行时环境变量注入，**不要 commit**：
  ...
  - **推荐配置已翻转为代码默认（v3 M4b，最优即默认，env 只用于回退）**：
    - thinking 默认 `disabled`（隐藏思维链占 43-91% completion_tokens，机械分类 24.8s→1.4s）；`LLM_THINKING=passthrough` 退回"不传参数"旧行为
    - 深度推理（swot/recommendations/reviewer_r6/judge/intake 流式）默认 `enabled`；`LLM_THINKING_DEEP=inherit` 退回"跟随全局"。**Seed-2.0-lite 传 auto 报 400**，llm.py 对 thinking 被拒自动去参重试，不会打成 degraded
    - payload 瘦身默认开（derivations prompt −23~38%、recommendations 74s→40s、judge 盲评零掉分）；`ANALYZER_PROMPT_SLIM=0` 退回全量 payload；紧口径微调 `ANALYZER_DERIV_MAX_PER_TYPE=5`/`ANALYZER_DERIV_SNIPPET_LEN=140`
    - 池内回捞默认开（缺口先回捞被 top-K/截断挡住的池内证据再外搜）；`ANALYZER_POOL_RECALL=0` 退回纯外搜
