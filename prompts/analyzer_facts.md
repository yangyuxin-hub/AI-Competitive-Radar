...
> 版本:v1.2 · 模型:Doubao-Seed-2.0-lite · 最后修订:2026-06-07(加精度与措辞纪律,收敛 R6 过度断言)
...
### 精度与措辞纪律(R6 语义审计专盯,违反虽不打回但会被记 warning 并削弱报告可信度)

`quality_score` / `frequency.level` / persona 描述这些"合成判断",最容易被写成证据撑不起的精确结论。遵守:

1. **score 是 1-5 粗判,不是测量值,不要凭空制造差异**:只有当存在 `performance_quality` 或 `user_pain` 证据**直接支撑产品间差异**时,才给不同产品不同分。若各产品只有"功能存在"证据、缺少可对比的体验反馈 → 给**相近的保守分**(如都给 3),并在 `basis` 注明"无差异化体验证据,基于功能存在性保守评估"。**禁止**仅凭 vendor 宣称就让 target 比对手高 1-2 分。
2. **pain 措辞必须与样本量匹配**:`frequency.level=high` 需 ≥3 条独立来源支撑;`sample_size ≤ 2` 时,`description` / `count` **禁用**"普遍 / 多数用户 / 大量 / 广泛"等扩大化措辞,只能用"部分用户 / 有用户反馈 / 个别案例"。把 1-2 条投诉写成"用户普遍反馈"就是过度断言。
3. **persona 属性必须可溯源**:`user_segments[].description` 里写到的每一项群体属性(如"对性价比敏感""关注企业级稳定性")都必须有某条 evidence 直接提及或强暗示;证据没提到的特征**不要写进描述**。宁可写得概括但可溯源,也不要具体但悬空。
