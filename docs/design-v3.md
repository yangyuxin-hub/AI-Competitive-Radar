...
> 状态：**部分被取代**——M0(StageReport 契约)/M1(timeline+checklist)已完成并保留;
> M2-M4(控制环统一/Collector 图节点拆分/收尾)被 `docs/design-v3-draft.md`(六角色重切)取代,
> 依据 54 run 实测打回触发率 1/54——外环按数据删除,不再投入统一控制环。Collection 真接缝结论下沉为 EvidenceService 内部结构。
