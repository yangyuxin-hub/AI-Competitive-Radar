...
_FACTS_SECTIONS = {
    ...
    "pricing_model": {
        ...
        "instruct": "本次任务只输出 `pricing_model` 这一个顶层字段(覆盖所有有定价证据的产品 + pricing_gap)。"
                    ...
                    "- 同时为每个 product 尽量输出通用定价引擎字段(全可选,缺证据就省略/空):\n"
                    "  * `pricing_structure.layers`:对象数组,mechanism 只能是 "
                    "`freemium`/`subscription`/`credits`/`one_time_packs`/`usage`,"
                    "每项含 `{mechanism,present,unit,evidence_ids}`;按人/每用户/月计费时 "
                    "`subscription.unit=\"seat\"`。\n"
                    "  * tiers 中若有积分/月额度,写 `credit_grant:{amount,unit,cadence}`;"
                    "若同档有多个买法,可写 `billing_options:[{cycle,is_promo,price}]`。\n"
                    "  * 若证据给出积分/产出消耗率或按量单价,写 product 级 `consumption`;"
                    "没有就 `consumption: []`,不要估算。\n"
                    "  * 若证据给出邀请好友送额度、限时赠送、任务奖励等活动,写 product 级 "
                    "`promotions:[{kind,name,reward:{amount,unit},condition,is_recurring,evidence_ids}]`;"
                    "邀请好友奖励 kind=`referral_reward`。活动/赠送额度不要塞进订阅档 `credit_grant`,"
                    "避免污染常规单位成本。\n"
                    " 这些字段由本地确定性 pricing_engine 计算 archetype/comparison_axis/unit_costs,"
                    "所以你只填证据事实,不要手算单位成本。\n"
